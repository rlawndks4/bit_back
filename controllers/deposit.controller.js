    ...
    list: async (req, res, next) => {
        try {
            ...
            let columns = [
                ...
                `SUM(CASE WHEN deposits.method_type = 2 THEN deposits.brand_deposit ELSE 0 END) OVER (ORDER BY deposits.id) AS total_use_bizppurio_deposit`,
                `SUM(CASE WHEN deposits.method_type = 2 THEN deposits.deposit ELSE 0 END) OVER (ORDER BY deposits.id) AS total_use_send_deposit`,
                `msg_logs.msg`,
                `msg_logs.res_msg`,
                `msg_logs.ref_key`,
                `msg_logs.receiver`,
